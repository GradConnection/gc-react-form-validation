...
class DatePicker extends Component {
  ...
  render() {
    const { placeholder, disabled, handleInputValidation, onInputChange } = this.props;
    ...
    return (
      <div
        ...
        className={`gc-input__el gc-input__el--no-padding ${
          isActive && !disabled ? "gc-input__el--active" : ""
        }`}
      ...
      >
        <div className="gc-drop-down__value">
          <input
            ...
            onBlur={() => {
              handleInputValidation(this.state.value)
              this.setState({ isActive: false })}}
            onChange={onInputChange(this.state.value)}
          />
          <GCIcon
            kind="calendarIcon"
